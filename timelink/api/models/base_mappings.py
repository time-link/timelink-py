...
Originally these mappings are generated by the Kleio translator when
processing kleio source files and generating data for the Timelink database.

So the mappings bellow have the same information as the Kleio source files.
They are needed for situations where the Kleio source files are not available
and Kleio groups are generated programmatically, like in the case of importing
from other data files. See the tests in tests(test_api_models_db.py for examples.
