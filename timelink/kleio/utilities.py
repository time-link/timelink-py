...
def get_metadata_key(*args):
    ...
    d: Any
        A default value for the metadata key is absent of no metadata on the field


    """
    assert len(args) == 3 or len(args) == 4, f'Expected 3 or 4 arguments, got {len(args)} '
    o = args[0]
    assert issubclass(type(o),dataclass), f'First argument must be subclass of dataclass'
...
def entity_to_kleio(entity):
    s = f'{entity.kgroup}$'
    positional = [f for f in fields(entity) if f.metadata.get('positional',False) == True ]
    optional = [f for f in fields(entity) if f.metadata.get('positional',False) == False ]
    slash_needed = False
    for f in positional:
        #print(f'testing posicional {f.name}')
        if hasattr(entity,f.name):
            if f.name not in ['obs','kgroup','attributes','relations_in','relations_out','contains'] and getattr(entity,f.name) != None:
                if slash_needed: 
                    s = s + '/'
