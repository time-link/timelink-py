...
async def login_form_post(request: Request, form: Annotated[LoginForm, fastui_form(LoginForm)]) -> list[AnyComponent]:
    webapp = request.app.state.webapp
    user_db = webapp.users_db
    user = authenticate_user(user_db, form.email, password=form.password.get_secret_value())
...
async def profile(request: Request, user: Annotated[UserSchema, Depends(get_current_user)]) -> list[AnyComponent]:
    return home_page(
