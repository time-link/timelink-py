        ...
        with navbar.header():
            ...
            with ui.row():
                self.text_input = ui.input(
                    label='You can search for a name, a job, or someting else.', placeholder='e.g. john smith blacksmith coimbra').on('keydown.enter', self._handle_searches).classes('w-80 ml-3')
                ui.button('Search').on('click', self._handle_searches).classes("items-center ml-3 mt-2")
            
            self.selector = ui.radio(["All Entities", "Real Persons", "Persons", "Acts"], value="All Entities").props('inline').classes("w-full")

            ui.label("Advanced Search").on('click', lambda : ui.navigate.to(f'/advanced_search')).classes('text-blue-500 ml-3 mb-2 cursor-pointer text-sm underline italic')
            ...
            with ui.row().classes("items-center w-full"):
                ...
                with ui.column().classes("items-center"):
                    self.sql_input = ui.textarea(
                        ...
                        ).props('dense outlined input-class="pl-2 pt-2"').classes("w-[60vh] bg-gray-50 border border-gray-300 rounded resize-none")
    ...
    async def advanced_search(self):
        with navbar.header():
            ...
            self.name_input = ui.input(
                label='Search Names', placeholder='e.g.: John Smith, \"John Andrew Smith\"').on('keydown.enter', self._handle_name_search).classes('w-80 items-center ml-3 mb-4')  
            ...
            with ui.row().classes("ml-3 -mt-5"):
                with ui.input('From') as self.from_date:
                    with ui.menu().props('no-parent-event') as menu:
                        with ui.date(on_change=menu.close).props(f"default-year-month=1600/01/01").bind_value(self.from_date):
                ...
                with ui.input('To') as self.to_date:
                    with ui.menu().props('no-parent-event') as menu:
                        with ui.date(on_change=menu.close).props(f"default-year-month=1700/12/31").bind_value(self.to_date):
                            with ui.row().classes('justify-end'):
                                ui.button('Close', on_click=menu.close).props('flat')
            ...
            ui.label("You can search up to six attributes. Use the % wildcard to search for any value.").classes("ml-3 mt-2 text-xs italic")
            ...
            for i in range(6):
                with ui.row().classes("ml-3 -mt-2"):
                    self.attr_select = ui.select(
                            ...
                            on_change= lambda new_val, index=i: self._attr_index_list_update(index, new_val.value)).props('dense outlined input-class="pl-3"').classes('w-[300px] bg-blue-50 rounded border border-blue-300')
                    ui.input(on_change= lambda new_val, index=i: self._attr_value_list_update(index, new_val.value)).props('outlined dense').classes('w-[200px]')

            
            with ui.row().classes("ml-3 -mt-2"):
                with ui.input('From') as self.attr_from_date:
                    with ui.menu().props('no-parent-event') as menu:
                        with ui.date(on_change=menu.close).props(f"default-year-month=1600/01/01").bind_value(self.attr_from_date):
                            with ui.row().classes('justify-end'):
                                ui.button('Close', on_click=menu.close).props('flat')
                ...
                with ui.input('To') as self.attr_to_date:
                    with ui.menu().props('no-parent-event') as menu:
                        with ui.date(on_change=menu.close).props(f"default-year-month=1700/12/31").bind_value(self.attr_to_date):
                            with ui.row().classes('justify-end'):
                                ui.button('Close', on_click=menu.close).props('flat')
            ...
            with ui.row().classes("ml-3"):
                self.text_input = ui.input(
                    label='Search by ID', placeholder='ID of something (person, source, act, etc..)').on('keydown.enter', self._handle_id_search).classes('w-80 items-center mb-4')
