        ...
        with navbar.header():
            ...
            with ui.row():
                self.text_input = ui.input(
                    label='Explore Database', placeholder='ID of something (person, source, act, etc..)').on('keydown.enter', self._handle_id_search).classes('w-80 items-center mb-4')
            ...
            with ui.row():
                for letter in string.ascii_uppercase + '?':
                    ui.label(letter)\
                        .on('click', lambda _, clicked_letter=letter: ui.navigate.to(f'/all_tables/persons?display_type=names&value={clicked_letter}'))\
                ...
                if attr_df is not None and not attr_df.empty:
                    ...
                    for result in attr_df["the_type"].unique():
                        with ui.row().classes("items-center gap-2 no-wrap"):
                            ui.label(result).on('click', lambda _, clicked_attr=result: ui.navigate.to(f'/all_tables/attributes?display_type=statistics&value={clicked_attr}'))\
                            ...
                            ui.label(str(grouped.get(result, 0))).on('click', lambda _, clicked_attr_count=result: ui.navigate.to(f'/tables/attributes?attr_type={clicked_attr_count}'))\
            ...
            with ui.row():
                ...
                if func_df is not None and not func_df.empty:
                    ...
                    for result in func_df["the_value"].tolist():
                        with ui.row().classes("items-center gap-2 no-wrap"):
                            ui.label(result).on('click', lambda _, clicked_func=result: ui.navigate.to(f'/tables/functions?type={clicked_func}'))\
                                .tooltip(f"Show all the values with {result}").classes('highlight-cell mb-4 cursor-pointer text-lg text-blue-500 font-semibold underline')
            ...
            with ui.row():
                ...
                if rel_df is not None and not rel_df.empty:
                    ...
                    for result in rel_df["the_type"].tolist():
                        with ui.row().classes("items-center gap-2 no-wrap"):
                            ui.label(result).on('click', lambda _, clicked_rel=result: ui.navigate.to(f'/all_tables/relations?display_type=statistics&value={clicked_rel}'))\
                                .tooltip(f"Show all the values with {result}").classes('highlight-cell mb-4 cursor-pointer text-lg text-blue-500 font-semibold underline')
                            ui.label(str(grouped.get(result, 0))).on('click', lambda _, clicked_rel_count=result: ui.navigate.to(f'/tables/relations?type={clicked_rel_count}'))\
                                .tooltip(f"Show all results with {result}").classes('highlight-cell -mt-10 cursor-pointer text-xs text-blue-500 font-semibold underline')
                else:
                    ui.label("No data found.").classes('mb-4 italic underline text-gray-500')
            ...
            with ui.row():
                self.attribute_type_search = ui.input(label='Choose Attribute Type', placeholder='Write an attribute type here...').on('keydown.enter', self._handle_advanced_search).classes('w-80 items-center mb-4 mr-3')
                self.attribute_value_search = ui.input(label='Choose Attribute Value', placeholder='Write an attribute value here...').on('keydown.enter', self._handle_advanced_search).classes('w-80 items-center mb-4 mr-3')
                ui.button('Show Persons').on('click', self._handle_advanced_search).classes("items-center mt-4 ml-3 mb-4")
            
            # Show database sources.
            ui.label("Show Sources").on('click', lambda sources: ui.navigate.to(f'/all_tables/sources?display_type=sources&value={sources}')).classes('mb-4 cursor-pointer text-orange-500 text-xl underline')
