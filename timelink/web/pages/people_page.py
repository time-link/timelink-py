...
class PeopleGroupsNetworks:
            ...
            with ui.card().tight().classes("w-full border-0 border-gray-300 rounded-none shadow-none"):
                ...
                with ui.tab_panels(self.tabs, value=self.people_tab).classes('w-full'):
                    with ui.tab_panel(self.people_tab):
                        with ui.row().classes(
                            "w-full bg-blue-100 text-lg rounded text-orange-500 font-bold flex justify-center items-center p-4"
                        ):
                            ...
                            ui.button(
                                ...
                                'click',
                                lambda: self._find_people_with_attributes(
                                    option_select.value,
                                    search_type.value,
                                    search_val.value
                                )
    ...
    def people_table_display(self, pd_results: pd.DataFrame, option: str) -> None:
        ...
        display_grid.on(
            ...
            if e.args["colId"] == "id" or
            e.args["colId"] == "destination_id"
            else None
        )
    ...
    async def _find_people_with_attributes(self, option, type, value):
        ...
        if option is None or type is None or value is None:
